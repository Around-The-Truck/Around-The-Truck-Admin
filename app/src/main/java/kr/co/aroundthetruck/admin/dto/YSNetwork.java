...
//    [{"menuIdx":"1", "price":"3000", "type":"0"},{"menuIdx":"2","price":"4000", "type":"1"}]
    public void setPayment(String dataList, String truckIdx, String customerPhone){
        AsyncHttpClient client = new AsyncHttpClient();
        RequestParams param = new RequestParams();
        Log.d("YoonTag", "서버 통신");

        try {
            Log.d("YoonTag", ""+dataList);
            Log.d("YoonTag", ""+truckIdx);
            Log.d("YoonTag", ""+customerPhone);
            param.put("menuArr", dataList);
            param.put("truckIdx", truckIdx);
        ...
        try {
            client.post("http://165.194.35.161:3000/pay", param, new AsyncHttpResponseHandler() {
                @Override
                public void onSuccess(int i, Header[] headers, byte[] bytes) {
                    Log.d("YoonTag", bytes.toString());
                    Log.d("YoonTag", new String(bytes));
